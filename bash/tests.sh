...
[ -f /etc/resolv.conf ] && echo "/etc/resolve.conf is a regular file" || echo "it's not a regular file"
[ -d /etc/resolv.conf ] && echo "/etc/resolve.conf is a directory" || echo "it's not a directory"
[ -h /etc/resolv.conf ] && echo "/etc/resolve.conf is a symbolic link" || echo "it's not a symbolic link"
[ -r /etc/resolv.conf ] && echo "/etc/resolve.conf is readable" || echo "it's not readable"
[ -x /etc/resolv.conf ] && echo "/etc/resolve.conf is executable" || echo "it's not writable"


echo "next section for testing"
...
read secondNumber
remainderFirst=$(($firstNumber%2))
remainderSecond=$(($secondNumber%2))

multipleOfFirst=$(($secondNumber%$firstNumber))


[ $remainderFirst -eq 1 ] && echo "the first number is odd" || echo "the first number is even"
[ $remainderSecond -eq 1 ] && echo "the second number is odd" || echo "the second number is even"

[ $multipleOfFirst -eq 0 ] && echo "the second number is a multiple of the first number" || echo "the second number is not a multiple of the first"
...
[ -n "$USER" ] && echo "the variable contains $USER" || echo "the variable is empty"
[ -z "$USER" ] && echo "the variable exists but is empty" || echo "the variable is not empty"
